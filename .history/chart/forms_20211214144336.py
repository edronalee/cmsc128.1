...
class TelemedForm(ModelForm):
    emailset = Account.objects.filter(groups__name='Doctors')
    nameset= {x.getFullName() for x in emailset}
    
    class Meta:
        model = Patient
        fields = ('telemed',)
        widgets = {
            'telemed': forms.ChoiceField(choices={x.getFullName() for x in Account.objects.filter(groups__name='Doctors')})
        }
