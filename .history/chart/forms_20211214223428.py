...
#LIST OF DOCTORS
class DoctorChoice(forms.ModelChoiceField):
    form = Self
    form.queryset = Account.objects.filter(groups__name = str("'Doctors'"))

class TelemedForm(ModelForm):
    class Meta:
        model = Patient
        nameset = Account.objects.filter(groups__name = str("'Doctors'"))
        telemed = forms.ModelChoiceField(queryset=nameset)
        fields = ('telemed',)
        widgets = {
            telemed:forms.Select(attrs={'class':'form-control'})
        }
