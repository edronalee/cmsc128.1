...
class TelemedForm(ModelForm):
    emailset = Account.objects.filter(groups__name='Doctors').values()
    nameset = {x['firstname'] + ' ' + x['lastname'] for x in emailset}
    class Meta:
        model = Patient
        fields = ('telemed',)
        widgets = {
            'telemed': forms.ModelChoiceField(choices = {x['firstname'] + ' ' + x['lastname'] for x in Account.objects.filter(groups__name='Doctors').values()}),
        }
